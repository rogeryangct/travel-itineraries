...
每份行程應有每日時段、移動時間、費用幣別與單位、Google Maps、景點背景與史實／傳說區分、官方來源、查核日期、預約、雨天備案、延誤刪減門檻及出發清單。舊天氣快照不等於即時預報。匯率、費用與營業變更須再查核。
...
行程內工具列保留日期、大字、列印／PDF；搜尋在全站頂部。大字僅放大行程閱讀內容，功能按鍵尺寸固定。分享可複製章節網址。
